...
aichess::aichess(QWidget* parent): aiboard(parent)
{
    setMouseTracking(true);
    //QRandomGenerator(QTime(0, 0, 0).secsTo(QTime::currentTime()));
    //connect(this, &aichess::chessPlayingDone, this, &aichess::chessPlaying);
    createNewChessMap();
}
...
void aichess::paintEvent(QPaintEvent* e)
{
    Q_UNUSED(e);
...
void aichess::mouseMoveEvent(QMouseEvent* e)
{
    int cx = e->x() - startX;
    int cy = e->y() - startY;
...
void aichess::mousePressEvent(QMouseEvent* e)
{
...
void aichess::chessPlaying()
{
    ...
    else if (winnerJudgment())
    {
        QString str = (pieceMode == Black) ? "黑方获胜！" : "白方获胜！";
        check(str);
    }
...
void aichess::check(QString str){
...
void aichess::returnmenu(){
    MainWindow *w = new MainWindow();
    w->show();
    this->close();
}

bool aichess::winnerJudgment()
{
...
void aichess::AIAttack(QVector<int> categorys)
{

}

QVector<int> aichess::roundErgodic(int x, int y, int pm)
{
...
QVector<int> aichess::typesConvert(QVector<int> types, int pm)
{
...
int aichess::WeightedCalculation(QVector<int> categorys, int pm)
{
    int weight = 0;
...
void aichess::createNewChessMap()
{
    pieceMode = Black;
...
QVector<int> aichess::saveChessMap()
{
    QVector<int> map;
    map.append(gameMode);
    map.append(cellNum);
